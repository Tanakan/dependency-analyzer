...
export const Graph: React.FC<GraphProps> = ({ data, selectedNode, selectedRepository }) => {
  ...
  useEffect(() => {
    ...
    // Create simulation (not used for now, but kept for potential future use)
    ...
    function updateLinks() {
      ...
      link.attr('d', d => {
        ...
        // Node radius (12 for circles, ~17 for rectangles)
        const sourceRadius = source.packaging === 'pom' ? 17 : 12;
        const targetRadius = target.packaging === 'pom' ? 17 : 12;
        
        // Adjust start and end points to node boundaries
        const sourceX = source.x! + nx * sourceRadius;
        const sourceY = source.y! + ny * sourceRadius;
        const targetX = target.x! - nx * (targetRadius + 5); // +5 for arrow head
        const targetY = target.y! - ny * (targetRadius + 5);
        
        // Calculate arc radius for curved path
        const dr = distance / 2;
...
function positionNodesInRepositories(
  ...
  repoLayouts.forEach((layout, repoName) => {
    ...
    // const nodeSpacing = 100; // Currently not used
